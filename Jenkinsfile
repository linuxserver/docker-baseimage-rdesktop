          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    docker manifest push --purge ${MANIFESTIMAGE}:fedora-40 || :
                    docker manifest create ${MANIFESTIMAGE}:fedora-40 ${MANIFESTIMAGE}:amd64-fedora-40 ${MANIFESTIMAGE}:arm64v8-fedora-40
                    docker manifest annotate ${MANIFESTIMAGE}:fedora-40 ${MANIFESTIMAGE}:arm64v8-fedora-40 --os linux --arch arm64 --variant v8
                    ...
                    digest=$(curl -s \
                      ...
                      "https://ghcr.io/v2/linuxserver/${CONTAINER_NAME}/manifests/arm32v7-fedora-40")
                    if [[ $(echo "$digest" | jq -r '.layers') != "null" ]]; then
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-fedora-40 || :
                      docker manifest create ${MANIFESTIMAGE}:arm32v7-fedora-40 ${MANIFESTIMAGE}:amd64-fedora-40
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-fedora-40
                    fi
                    docker manifest push --purge ${MANIFESTIMAGE}:fedora-40
                    docker manifest push --purge ${MANIFESTIMAGE}:${META_TAG} 
