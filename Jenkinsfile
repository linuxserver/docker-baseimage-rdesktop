pipeline {
  ...
  stages {
    ...
    stage("Set ENV Variables base"){
      steps{
        ...
        script{
          env.EXIT_STATUS = ''
          env.LS_RELEASE = sh(
            script: '''docker run --rm quay.io/skopeo/stable:v1 inspect docker://ghcr.io/${LS_USER}/${CONTAINER_NAME}:bionic 2>/dev/null | jq -r '.Labels.build_version' | awk '{print $3}' | grep '\\-ls' || : ''',
    ...
    // If this is a bionic build check the S6 service file perms
