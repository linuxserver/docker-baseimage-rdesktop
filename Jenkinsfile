pipeline {
  ...
  stages {
    ...
    // If this is a debianbookworm build use live docker endpoints
    stage("Set ENV live build"){
      when {
        branch "debianbookworm"
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              # ${TEMPDIR}/docker-${CONTAINER_NAME}: Cloned branch debianbookworm of ${LS_USER}/${LS_REPO} for running the jenkins builder on
              # ${TEMPDIR}/repo/${LS_REPO}: Cloned branch debianbookworm of ${LS_USER}/${LS_REPO} for commiting various templated file changes and pushing back to Github
              # ${TEMPDIR}/docs/docker-documentation: Cloned docs repo for pushing docs updates to Github
              # ${TEMPDIR}/unraid/docker-templates: Cloned docker-templates repo to check for logos
              # ${TEMPDIR}/unraid/templates: Cloned templates repo for commiting unraid template changes and pushing back to Github
              git clone --branch debianbookworm --depth 1 https://github.com/${LS_USER}/${LS_REPO}.git ${TEMPDIR}/docker-${CONTAINER_NAME}
              ...
              if [[ "$(md5sum Jenkinsfile | awk '{ print $1 }')" != "$(md5sum ${TEMPDIR}/docker-${CONTAINER_NAME}/Jenkinsfile | awk '{ print $1 }')" ]]; then
                ...
                git checkout -f debianbookworm
                cp ${TEMPDIR}/docker-${CONTAINER_NAME}/Jenkinsfile ${TEMPDIR}/repo/${LS_REPO}/
                git add Jenkinsfile
                git commit -m 'Bot Updating Templated Files'
                git pull https://LinuxServer-CI:${GITHUB_TOKEN}@github.com/${LS_USER}/${LS_REPO}.git debianbookworm
                git push https://LinuxServer-CI:${GITHUB_TOKEN}@github.com/${LS_USER}/${LS_REPO}.git debianbookworm
    ...
    // If this is a debianbookworm build check the S6 service file perms
      ...
      steps {
        sh '''#! /bin/bash
              ...
              if [ "${NEW_PACKAGE_TAG}" != "${PACKAGE_TAG}" ]; then
                ...
                git --git-dir ${TEMPDIR}/${LS_REPO}/.git checkout -f debianbookworm
          ...
          retry_backoff(5,5) {
            sh '''#! /bin/bash
                  ...
                  for PUSHIMAGE in "${GITHUBIMAGE}" "${GITLABIMAGE}" "${QUAYIMAGE}" "${IMAGE}"; do
                    ...
                    docker tag ${PUSHIMAGE}:${META_TAG} ${PUSHIMAGE}:debianbookworm
                    docker tag ${PUSHIMAGE}:${META_TAG} ${PUSHIMAGE}:${EXT_RELEASE_TAG}
                    if [ -n "${SEMVER}" ]; then
                      docker tag ${PUSHIMAGE}:${META_TAG} ${PUSHIMAGE}:${SEMVER}
                    fi
                    docker push ${PUSHIMAGE}:debianbookworm
          ...
          retry_backoff(5,5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    docker tag ${MANIFESTIMAGE}:amd64-${META_TAG} ${MANIFESTIMAGE}:amd64-debianbookworm
                    docker tag ${MANIFESTIMAGE}:amd64-${META_TAG} ${MANIFESTIMAGE}:amd64-${EXT_RELEASE_TAG}
                    docker tag ${IMAGE}:arm64v8-${META_TAG} ${MANIFESTIMAGE}:arm64v8-${META_TAG}
                    docker tag ${MANIFESTIMAGE}:arm64v8-${META_TAG} ${MANIFESTIMAGE}:arm64v8-debianbookworm
                    ...
                    docker push ${MANIFESTIMAGE}:amd64-debianbookworm
                    docker push ${MANIFESTIMAGE}:arm64v8-${META_TAG}
                    docker push ${MANIFESTIMAGE}:arm64v8-debianbookworm
      ...
      steps {
        ...
        sh '''#! /bin/bash
              ...
              echo '{"tag_name":"'${META_TAG}'",\
                     "target_commitish": "debianbookworm",\
                     "name": "'${META_TAG}'",\
    ...
    stage('Github-Release-Branch-Protection') {
      ...
      steps {
        echo "Setting up protection for release branch debianbookworm"
        sh '''#! /bin/bash
          curl -H "Authorization: token ${GITHUB_TOKEN}" -X PUT https://api.github.com/repos/${LS_USER}/${LS_REPO}/branches/debianbookworm/protection \
          -d $(jq -c .  << EOF
            {
